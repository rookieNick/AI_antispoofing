# ==============================================================================
# Patch-based CNN Testing Script for Face Anti-Spoofing
# ==============================================================================
# This script tests patch-based CNN models on the test dataset and generates
...
MODEL_TYPE = 'patch'          # 'patch' or 'patch_depth' - must match training
PATCH_SIZE = 32               # Size of each patch (must match training)
NUM_PATCHES = 9               # Number of patches (must match training)
SAMPLE_SIZE = -1              # Number of samples to test (-1 means full dataset)

# Model Configuration
MODEL_FILENAME = 'patch_cnn2.pth'
...
def test_patch_model():
    """
    Main testing function for patch-based CNN models
    """
    print("=" * 80)
    print(f"TESTING {MODEL_TYPE.upper()} PATCH-BASED CNN FOR FACE ANTI-SPOOFING")
    print("=" * 80)
    ...
    if not os.path.exists(model_path):
        ...
        print("Please train the model first using train_patch_cnn.py")
        return
    ...
    # --- Model Setup ---
    print(f"\\n🏗️  Loading {MODEL_TYPE} patch-based CNN model...")
    ...
    criterion = nn.CrossEntropyLoss() # Assuming CrossEntropyLoss is appropriate
    ...
    base_name = f"test_{MODEL_TYPE}_patch_{index}_{date_str}"
